# Statistical Learning with Python

A collection of statistical learning notebooks implemented in Python.

## Notebooks

- **01_intro.ipynb** - Introduction to statistical computing and data visualization
- **02_regression.ipynb** - Linear regression with statsmodels
- **03_classification_methods.ipynb** - Different classification models

## License

MIT License - see [LICENSE](LICENSE) file for details.

## Acknowledgments

This project is a Python adaptation inspired by the R statistical learning labs 
from Vadim Smolyakov's repository.
